...
_P2 = [cp.eye(3),
       ...
       cp.flipud(cp.eye(3)),
       cp.rot90([[0, 1, 0]] * 3)]
_P3 = [cp.zeros((3, 3, 3)) for i in range(9)]
...
def sup_inf(u):
    ...
    erosions = []
    for P_i in P:
        erosions.append(ndi.binary_erosion(u, P_i).astype(cp.int8))
...
def inf_sup(u):
    ...
    dilations = []
    for P_i in P:
        dilations.append(ndi.binary_dilation(u, P_i).astype(cp.int8))
