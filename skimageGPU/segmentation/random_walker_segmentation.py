import cupy as cp
from cupyx.scipy import sparse, ndimage as ndi
...
def _compute_weights_3d(data, spacing, beta, eps, multichannel):
    ...
    if multichannel:
        ...
        scale_factor /= cp.sqrt(data.shape[-1])
...
def _make_graph_edges_3d(n_x, n_y, n_z):
    ...
    edges_deep = cp.vstack((vertices[..., :-1].ravel(), vertices[..., 1:].ravel()))
    edges_right = cp.vstack((vertices[:, :-1].ravel(), vertices[:, 1:].ravel()))
    edges_down = cp.vstack((vertices[:-1].ravel(), vertices[1:].ravel()))
    edges = cp.hstack((edges_deep, edges_right, edges_down))
    return edges
...
def _build_laplacian(data, spacing, mask, beta, multichannel):
    ...
    data = cp.hstack((weights, weights))
...
def _build_linear_system(data, spacing, labels, nlabels, mask, beta, multichannel):
    ...
    seeds_mask = sparse.csc_matrix(
        cp.hstack([cp.atleast_2d(seeds == lab).T for lab in range(1, nlabels + 1)])
    )
...
def _preprocess(labels):
    ...
    if label_values[0] < 0 or cp.any(isolated):
        isolated = cp.logical_and(
            cp.logical_not(ndi.binary_propagation(pos_mask, mask=mask)), null_mask
        )
    ...
    zero_idx = cp.searchsorted(label_values, 0)
    ...
    *,
    ...
    if spacing is None:
        ...
        if len(spacing) == 2:
            ...
            spacing = cp.row_stack[spacing, 1.0]
        spacing = cp.asarray(spacing)
    else:
        raise ValueError(
    ...
    if isolated_values is None:
        ...
        if return_full_prob:
            ...
            return cp.concatenate(
                [cp.atleast_3d(labels == lab) for lab in cp.unique(labels) if lab > 0],
                axis=-1,
            )
        return labels
