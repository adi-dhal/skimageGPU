...
def active_contour(image, snake, alpha=0.01, beta=0.1,
    ...
    if RGB:
        img = w_line*cp.sum(img, axis=2) \
            + w_edge*sum(edge)
    else:
        img = w_line*img + w_edge*edge[0]

    # Interpolate for smoothness:
    intp = RectBivariateSpline(cp.arange(img.shape[1]),
                               cp.arange(img.shape[0]),
                               img.T, kx=2, ky=2, s=0)
    ...
    n = len(x)
    xsave = cp.empty((convergence_order, n), dtype=float_dtype)
    ysave = cp.empty((convergence_order, n), dtype=float_dtype)

    # Build snake shape matrix for Euler equation in double precision
    eye_n = cp.eye(n, dtype=float)
    a = (cp.roll(eye_n, -1, axis=0)
         + cp.roll(eye_n, -1, axis=1)
         - 2 * eye_n)  # second order derivative, central difference
    b = (cp.roll(eye_n, -2, axis=0)
         + cp.roll(eye_n, -2, axis=1)
         - 4 * cp.roll(eye_n, -1, axis=0)
         - 4 * cp.roll(eye_n, -1, axis=1)
    ...
    inv = cp.linalg.inv(A + gamma * eye_n)
    ...
    for i in range(max_num_iter):
        ...
        dx = max_px_move * cp.tanh(xn - x)
        dy = max_px_move * cp.tanh(yn - y)
        if sfixed:
            dx[0] = 0
            dy[0] = 0
        if efixed:
        ...
        if j < convergence_order:
            ...
            dist = cp.min(cp.max(cp.abs(xsave - x[None, :])
                                 + cp.abs(ysave - y[None, :]), 1))
            if dist < convergence:
                break

    return cp.stack([y, x], axis=1)
